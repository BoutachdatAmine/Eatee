...
@section('content')
<script src="{{ url('js/orderScript.js')}}"></script>

<div class="container mx-auto px-6 py-8">
  <h3 class="text-gray-700 text-3xl font-medium">Orders of {{$date}} </h3>
  <br>
      ...
      <table class="min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Pickup Date</th>
        ...
        <tbody class="bg-white">
          ...
          <tr>
            ...
            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
              <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{{$order->total}}</span>
            </td>

            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
              <div class="text-sm leading-5 text-gray-900"><?php echo (date("Y-m-d", strtotime($order->orderedAt))); ?></div>
            </td>

            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
              <div class="text-sm leading-5 text-gray-900"><?php echo (date("Y-m-d", strtotime($order->orderedFor))); ?></div>
            </td>
            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
            ...
            <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
              @if($order->status != "READY_FOR_PICKUP" && $order->status != "DONE")
              <a href="{{route('showOrder',['id' => $order->orderId])}}" class="px-6 py-3 bg-red-600 rounded-md text-white font-medium tracking-wide hover:bg-red-500">Check Order</a>
              @else
              <a class="px-6 py-3 bg-green-600 rounded-md text-white font-medium tracking-wide hover:bg-green-500">Done</a>
              @endif
...
<div id="changeModal" class="opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center">
  ...
  <div id="changeModal-container" class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
    ...
    <div id="changeModal-content" class="py-4 text-left px-6">
      ...
      <form class="mt-6" action="{{route('changeDateOrder')}}" method="POST">
        @csrf
